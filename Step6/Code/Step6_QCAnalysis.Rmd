...
physeq_filt_samples <- physeq_filt_samples %>% scale_reads(n=sample_depth) 
sample_data(physeq_filt_samples)$SampleType <- factor(
  sample_data(physeq_filt_samples)$SampleType, 
  levels = unique(sample_data(physeq_filt_samples)$SampleType)
)
...
if (include_reps = "Y"){
 ...
 physeq_pcoa_sub<-as.data.frame((physeq_pcoa$vectors[,1:5])) #select PC1-PC5
 physeq_pcoa_sub<-merge(physeq_pcoa_sub,metadata[,c("RunID","SubjectID")],by=0)
 p<-ggpairs(physeq_pcoa_sub,columns=2:6,
         mapping= ggplot2::aes(colour=SubjectID))
