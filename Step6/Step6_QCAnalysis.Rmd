...
#3 Prune only study and only controls
```{r}
#Only bacteria
...
#5. Filter taxa >.001 and samples with less than 500 reads
```{r}
...
#7 Plot PCOA Data - All samples (Sample Type, Subject)
```{r}
...
sample_data(physeq_scale)$Subject <- factor(
  sample_data(physeq_scale)$Subject, 
  levels = unique(sample_data(physeq_scale)$Subject)
)
...
#Ordinate for subject
physeq_scale_samples <- physeq_filt %>%
  subset_samples(
    Subject != "Control" 
  )
physeq_pcoa <- ordinate(
...
p<- plot_ordination(
  ...
  ordination = physeq_pcoa,
  color = "Subject",
  title = "PCoA by Subject (Bray-Curtis)"
)  +
  geom_point(aes(color = Subject), alpha = 0.7, size = 6) +
  geom_point(colour = "grey90", size = 1.5)

#Save file
file_name =("Graphs/subjectid_bray.tiff")
tiff(file_name, width=800)
print(p)
dev.off()
...
#9 View top Genus - All samples  by Subject
```{r}
...
#Extraction Batch
p<- plot_richness(physeq_scale, x="Extraction.Batch.ID") + geom_boxplot()
...
#Collection Method
p<- plot_richness(physeq_scale, x="Collection.Method") + geom_boxplot()
...
physeq_subset<- subset_samples(physeq_scale10, Storage=="80")
p<- plot_bar(physeq_subset, "Instrument", fill="Genus", facet_grid=~Collection.Method~Month, title="Genus Abundances by Collection Type/ Month/ Instrument")+geom_bar(stat="identity")

#Save file
file_name =("Graphs/summary_genus_all.tiff")
tiff(file_name, width=800)
print(p)
dev.off()

#Overall comparison at -20*C
physeq_subset<- subset_samples(physeq_scale10, Month=="ExtMonth0")
p<- plot_bar(physeq_subset, "Instrument", fill="Genus", facet_grid=~Storage~Collection.Method, title="Genus Abundances by Collection Type/ Month/ Instrument")+geom_bar(stat="identity")
